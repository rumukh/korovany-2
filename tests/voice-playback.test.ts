...
import { SpeechPresentation, type SpeechOutput } from "../src/audio/presentation";
import { factions, journalBefore, scene, voiceState } from "../scripts/voices/catalogue";

function output(): SpeechOutput {
  return { speak: vi.fn(), cancelSpeech: vi.fn(), setSpeechActive: vi.fn() };
}
...
describe("recorded faction voice presentation", () => {
  ...
  test.each(factions)("%s: queues the selected player line before exact NPC text in both languages", faction => {
    ...
    const sound = output(), presentation = new SpeechPresentation(sound);
    ...
    for (const language of ["ru", "en"] as const) {
      ...
      expect(sound.setSpeechActive).toHaveBeenLastCalledWith(true);
    }
  });
  ...
  test("pause/focus/title cancel audibility without changing game state or losing scene identity", () => {
    ...
    expect(sound.setSpeechActive).toHaveBeenLastCalledWith(false);
