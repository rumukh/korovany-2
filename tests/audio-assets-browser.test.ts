...
describe.runIf(process.env.KOROVANY_BROWSER === "1")("shipped Ogg audio under a deployment prefix", () => {
  ...
  it("loads the complete approved faction voice manifest alongside the soundtrack", async () => {
    const manifest = parseVoices(await (await fetch(new URL("audio/voices/manifest.json", origin))).json());
    expect(manifest.size).toBe(1332);
    expect(new Set([...manifest.values()].map((entry) => entry.speaker)).size).toBe(22);
    const clips = [...manifest.values()].flatMap((entry) => entry.clips);
    expect(clips).toHaveLength(3772);
    expect(new Set(clips.map(clip => clip.src)).size).toBe(3115);
    for (const language of ["ru", "en"]) {
      expect([...manifest.values()].filter((entry) => entry.language === language)).toHaveLength(666);
    }
    const scorePaths = new Set([...soundtrack.music, ...soundtrack.ambience, ...soundtrack.sfx].map(asset => asset.src));
    expect(clips.every(clip => !scorePaths.has(clip.src))).toBe(true);
    // voice-assets-browser owns the single exhaustive decode of all 3,115 approved clips.
  });
  ...
  it("plays real ending music and terminal cues through the production transport", async () => {
    ...
    try {
      for (const faction of ["elf", "guard", "villain"] as const) for (const [id, title] of Object.entries(getFactionStory(faction).endings)) {
        const terminal: GameSnapshot = structuredClone(snapshot);
        terminal.runId = `audio-${faction}-${id}`;
        ...
        expect(audio.music).toBe(`ending-${id}`);
