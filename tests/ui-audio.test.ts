...
class Context {
  ...
  decodeAudioData = vi.fn(async (_bytes: ArrayBuffer) => ({ length: 48_000, numberOfChannels: 1 }));
  createGain = () => {
    const gain = new Gain();
...
describe("cancellable bilingual speech", () => {
  ...
  it("bounds unabortable native decodes when rapid speech selections replace each other", async () => {
    ...
    await sound.unlock();
    await flush();
    const first = deferred<{ length: number; numberOfChannels: number }>();
    const second = deferred<{ length: number; numberOfChannels: number }>();
    const context = Context.instances[0]!;
    ...
    first.resolve({ length: 48_000, numberOfChannels: 1 });
    second.resolve({ length: 48_000, numberOfChannels: 1 });
    await flush();
    expect(sound.inspect().subtitle?.speaker).toBe("player");
    expect(context.sources).toHaveLength(1);
    expect(sound.inspect().cacheEntries).toBe(1);
...
describe("bounded spatial effects and memory", () => {
  ...
  it("evicts least-recently-used decoded audio without decoding streamed beds", async () => {
    ...
    await flush();
    const context = Context.instances[0]!;
    context.decodeAudioData.mockResolvedValue({ length: 1_000_000, numberOfChannels: 1 });
    for (let i = 0; i < 16; i++) {
      sound.cue(`effect${i}`);
      await flush();
      context.sources.at(-1)?.end();
