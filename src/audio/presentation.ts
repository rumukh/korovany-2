...
import { ENDINGS, type EndingId } from "../game/narrative-data";
...
export class AudioPresentation {
  ...
  constructor(private readonly sound: Soundscape) {}
  ...
  sync(snapshot: GameSnapshot | null, overlay: Overlay, atTitle: boolean, language: Language, selection?: SpeechSelection): void {
    ...
    const ending = snapshot?.narrative?.ending;
    const key = this.screen === "story" && dialogue ? `dialogue:${dialogue.npcId}:${dialogue.text.en}`
      : this.screen === "story" && inspection ? `inspection:${inspection.locationId}:${inspection.text.en}`
      : this.screen === "terminal" && ending ? `ending:${ending.en}` : this.screen;
  ...
  private beds(snapshot: GameSnapshot | null): void {
    ...
    if (snapshot.fortress.unlocked && !snapshot.fortress.bossDefeated &&
      snapshot.actors.some((actor) => actor.kind === "boss" && actor.hp > 0 && near(actor, 36))) this.bossUntil = snapshot.elapsed + 8;
