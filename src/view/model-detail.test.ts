...
import { createActor, createWagon, type ActorLook, type ViewAllegiance } from './actors';
...
describe('crafted frontier models', () => {
  test('retains textured detailed rigs and shared assets across explicit allegiances', () => {
    ...
    const resources = new ViewResources(loader, 8);
    const affiliations: ViewAllegiance[] = ['hostile', 'neutral', 'friendly'];
    let canopyGeometry: THREE.BufferGeometry | undefined;
    let clothMap: THREE.Texture | null = null;
    const material = (mesh: THREE.Object3D | undefined): THREE.MeshStandardMaterial => {
      if (!(mesh instanceof THREE.Mesh) || !(mesh.material instanceof THREE.MeshStandardMaterial)) {
        throw new Error('Expected a physically based model mesh');
      }
    ...
    try {
      for (const allegiance of affiliations) {
        ...
        expect(meshes(actor.root).every(mesh => mesh.customDepthMaterial === resources.depthMaterial()
          || mesh.name === 'allegiance-ring')).toBe(true);
        const wagon = createWagon(resources, allegiance);
        expect(wagon.root.userData.allegiance).toBe(allegiance);
        expect(wagon.root.getObjectsByProperty('name', 'wagon-wheel')).toHaveLength(4);
        const canopy = wagon.root.getObjectByName('wagon-canopy');
        const painted = material(canopy);
        expect(painted.map).toBeInstanceOf(THREE.Texture);
        expect(painted.normalMap).toBeInstanceOf(THREE.Texture);
        expect(painted.roughnessMap).toBeInstanceOf(THREE.Texture);
        if (!(canopy instanceof THREE.Mesh)) throw new Error('Missing detailed canopy');
        expect(canopy.geometry.getAttribute('position').count).toBeGreaterThan(300);
        canopyGeometry ??= canopy.geometry;
        clothMap ??= painted.map;
        expect(canopy.geometry).toBe(canopyGeometry);
        expect(painted.map).toBe(clothMap);
        const flag = meshes(wagon.root).find(mesh => mesh.geometry === resources.geometry('cloth', () => {
          throw new Error('Missing shared cloth geometry');
        }) && material(mesh).color.getHexString() === new THREE.Color(
          allegiance === 'friendly' ? palette.teal : allegiance === 'neutral' ? palette.stone : palette.hostile).getHexString());
        expect(flag).toBeDefined();
        expect(material(flag).map).toBe(clothMap);
      }
