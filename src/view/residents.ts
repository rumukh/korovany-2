...
function resident(resources: ViewResources, npc: NpcSnapshot): Resident {
  ...
  for (const side of [-1, 1]) {
    part(resources, body, 'box', palette.iron, [side * 0.17, 0.4, 0], [0.23, 0.7, 0.26]);
    part(resources, body, 'box', palette.bark, [side * 0.17, 0.12, 0.08], [0.27, 0.22, 0.39]);
  }
  part(resources, body, 'box', coat, [0, 1.13, 0], [0.65, 0.82, 0.43]);
  part(resources, body, 'cone', coat, [0, 0.72, 0], [0.78, 0.62, 0.57]);
  part(resources, body, 'box', palette.bark, [0, 0.92, 0.23], [0.7, 0.08, 0.035]);
  const head = joint(body, [0, 1.88, 0]);
  part(resources, head, 'sphere', palette.skin, [0, 0, 0.03], [0.43, 0.51, 0.42]);
  part(resources, head, 'box', palette.ink, [0, 0.045, 0.23], [0.27, 0.035, 0.018]);
  part(resources, head, 'box', palette.skin, [0, -0.02, 0.25], [0.08, 0.12, 0.055]);
  ...
  if (variant === 0) {
    ...
    part(resources, head, 'box', hair, [0, -0.17, 0.2], [0.3, 0.22, 0.1]);
  } else if (variant === 2) {
  ...
  for (const arm of [leftArm, rightArm]) {
    part(resources, arm, 'box', coat, [0, -0.22, 0], [0.24, 0.55, 0.28]);
