        ...
        for variant, item in report["capability_probe"]["variants"].items():
            ready.append({
                ...
                "note": f"Probe, not story audio. ASR: {item['transcription']}. Engine sentinel followed: "
                        f"{report['capability_probe']['sentinel_followed']}. Correct target scores: "
                        + "; ".join(f"{target['target']}={target['accuracy_score']}" for target in
                                    report["capability_probe"]["variants"]["correct"]["target_occurrences"])
