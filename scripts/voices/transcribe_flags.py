...
def transcribe(index_path, output, skill, workers=3, identifiers=None):
    ...
    def one(group):
        ...
        cached = []
        for member in group:
            destination = output / f"{member['id']}.json"
            if destination.exists():
                prior = load(destination)
                if prior["audio_sha256"] != audio_hash:
                    raise RuntimeError(f"Full-file ASR cache belongs to different audio: {member['id']}")
                cached.append(prior)
        result = cached[0] if cached else continuous_transcript(path, language, region, key)
        receipts = []
        for member in group:
            if member["sha256"] != audio_hash or digest(member["path"]) != audio_hash:
                raise RuntimeError(f"Shared ASR audio changed: {member['id']}")
            receipt = {**result, "id": member["id"], "audio_sha256": audio_hash,
                       "method": "Azure continuous full-file recognition", "expected_text": member["text"]}
            save(output / f"{member['id']}.json", receipt)
            receipts.append(receipt)
        return receipts

    groups = {}
    for item in selected:
        groups.setdefault((item["sha256"], "-".join(item["voice"].split("-")[:2])), []).append(item)
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = [executor.submit(one, group) for group in groups.values()]
