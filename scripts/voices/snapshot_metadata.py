...
def snapshot(inventory_path, prepared, auditions, output):
    ...
    save(output, {
        "version": 1, "status": "samples-ready-not-human-approved",
        ...
        "limitations": [
            ...
            f"Engines obeying sentinel phonemes: {sum(item['sentinel_followed'] for item in engines.values())}/{len(engines)}. Probe flags: "
            + ", ".join(engine for engine, item in engines.items() if item["probe_decision"] != "PASS"),
            "Automated pronunciation/recognition flags are retained for human listening. Thresholds are unchanged.",
            "Existing cast approval does not imply listening approval for these new recordings. Full generation additionally requires a source-bound authorization and acceptance of any flagged probe report.",
            "No automatic resynthesis or score chasing. NISQA scores do not approve words, acting, pronunciation, or final game-mix balance.",
        ],
    })
